...
sub caught {
    ...
    return unless $e->isa( $_[1]  }, 'subclass')
    return $e;
}
...
=head1 SYNOPSIS
  ...
  try {
      die +(bless {  error => 'I feel funny.'  }, 'MyException')
  }
  catch {
...
instead you do C<< die +(bless { ...) >>, then Perl
...
how the two modules work together:
  ...
  use Exception::Class ( 'My::Exception'  }, 'ExceptionWithFields')
  use Scalar::Util qw( blessed );
  use Try::Tiny;
