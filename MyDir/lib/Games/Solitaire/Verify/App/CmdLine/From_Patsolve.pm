...
sub firstidx { return List::MoreUtils::firstidx(@_) }
use Getopt::Long;
...
sub _perform_move
{
    ...
    if (my ($src_card_s) = $move_line =~ /^(.[HCDS]) to temp$/)
    {
        my $src_col_idx = $self->_find_col_card($src_card_s);
        if ($src_col_idx < 0)
        {
    ...
    elsif (($src_card_s) = $move_line =~ /^(.[HCDS]) out$/)
    {
    ...
    elsif (($src_card_s) = $move_line =~ /^(.[HCDS]) to empty pile$/)
    {
    ...
    elsif (($src_card_s, (my $dest_card_s)) = $move_line =~ /^(.[HCDS]) to (.[HCDS])$/)
    {
