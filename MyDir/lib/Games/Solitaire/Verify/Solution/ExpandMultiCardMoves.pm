...
use Games::Solitaire::Verify::Solution::Base (); use vars qw(@ISA); @ISA = (qw(Games::Solitaire::Verify::Solution::Base));

# TODO : Merge with lib/Games/Solitaire/Verify/Solution.pm

use POSIX;

sub ceil { return POSIX::ceil(@_) }
...
sub _output_fh
{
    ...
    if (@_)
    {
        $self->{'_output_fh'} = shift;
    }
    return $self->{'_output_fh'};
}
